...
int main() {
    ...
    //hModule=(HMODULE)((DWORD_PTR)hModule & 0x00FFFFFF);

    LPVOID fileContent = (LPVOID)((ULONG_PTR)hModule & ~((ULONG_PTR)0xffff));
    BOOL result = VerifyDllFromHModule((HMODULE)fileContent);

    if (result) {

        printf("The DLL is verified successfully.\n");
